...
def test_create_parking(app_client) -> None:
    parking_data = {
        "address": "123 Aleksanderkatu, Helsinki",
        "opened": True,
        "count_places": 20,
        "count_available_places": 18,
        "opening_time": "07:00",
        "closing_time": "21:00",
    }
    headers = {"Content-Type": "application/json"}
...
def test_check_in_parking(app_client, db) -> None:
    check_in_data = {"client_id": 1, "parking_id": 1, "time_in": "08:00"}
...
def test_check_in_parking_full(app_client, db):
    ...
    check_in_data = {"client_id": 3, "parking_id": 4}
    ...
    client_parking = (
        db.session.query(ClientParking).filter_by(client_id=3, parking_id=4).first()
    )
...
def test_check_out_parking(app_client, db) -> None:
    ...
    client = Client(
        ...
        name="Ahti",
        surname="Virtanen",
        credit_card="1234-5678-1234-3456",
        car_number="AM456BC",
    )
    db.session.add(client)
    ...
    check_in = ClientParking(
        client_id=5, parking_id=5, time_in=check_in_time, time_out=None
    )
