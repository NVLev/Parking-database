...
def create_app():
    ...
    db.init_app(app)
    from models.model import Base, Client, ClientParking, Parking, is_parking_open
            ...
            for parking_data in sample_parkings:
                new_parking = Parking(
                    ...
                    opening_time=datetime.strptime(
                        parking_data["opening_time"], "%H:%M"
                    ).time(),
                    closing_time=datetime.strptime(
                        parking_data["closing_time"], "%H:%M"
                    ).time(),
                )
    ...
    def delete_client_parking():
        ...
        if not check_in:
            return (
                jsonify(
                    {
                        "error": "Tälle asiakkaalle ja pysäköintipaikalle ei löytynyt aktiivista sisäänkirjautumista"
                    }
                ),
                404,
            )
